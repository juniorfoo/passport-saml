...
SAML.prototype.signRequest = function (samlMessage) {
  ...
  if (typeof(self.options.privateCert) === 'function') {
    return Q.nfcall(self.options.privateCert, samlMessage);
  } else {
    return Q.ninvoke(self, 'signRequestLocal', samlMessage);
  }
};

SAML.prototype.signRequestLocal = function (samlMessage, callback) {
  var signer;
    ...
    if (self.options.privateCert) {
      try {
        ...
        self.signRequest(samlMessage)
          .then(function (samlMessageSigned) {
            Object.keys(samlMessageSigned).forEach(function (k) {
              target.query[k] = samlMessageSigned[k];
            });
